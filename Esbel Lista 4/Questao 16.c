...
int quantosPixelsAcimaDeInt_D(unsigned char** img, int nLin, int nCol, unsigned char inten){
    int quantI = quantosPixelsNaInt_D(img, nLin, nCol, inten);
    int quantAB = quantosPixelsAbaixoDeInt_D(img, nLin, nCol, inten);
    int quantAC = nLin * nCol - quantAB - quantI;
    return quantAC;
}
...
int main(void){
    ...
    scanf("%d", &tipo);
    printf("Entre com o pixel que deseja utilizar:\n");
    scanf("%u", &inten);
    ...
    printf("Quantidade de pixeis %u na int: %d\n", inten, quantosPixelsNaInt_D(img, nLin, nCol, inten));
    printf("Quantidade de pixeis abaixo de %u na int: %d\n", inten, quantosPixelsAbaixoDeInt_D(img, nLin, nCol, inten));
    printf("Quantidade de pixeis acima de %u na int: %d\n", inten, quantosPixelsAcimaDeInt_D(img, nLin, nCol, inten));
    for(int i = 0; i < nLin; i++){
        free(img[i]);
    }
    free(img);
